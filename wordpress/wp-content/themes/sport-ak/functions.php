...
function azexo_admin_notices() {
    ...
    if (azexo_return_bytes(ini_get('memory_limit')) < (96 * 1024 * 1024)) {
        ?>
        <div class="error">
            <p><?php esc_html_e('Memory limit must be >= 96MB', 'sport-ak'); ?></p>
        </div>
        <?php
    }
}
...
function azexo_inline_js() {
    ...
    if (!empty($options['custom-js'])) {
        ...
        print 'var templateurl = "' . esc_js(get_bloginfo("template_url")) . '";';
        print '</script>';
    }
}
...
if (function_exists('register_nav_menus')) {
    register_nav_menus(array(
        ...
        'signature-steps' => esc_html__('Menu de assinaturas', 'sport-ak'),
...
function azexo_entry_field($name) {
    ...
    switch ($name) {
        ...
        case 'post_author_avatar':
            return '<span class="avatar">' . get_avatar(get_the_author_meta('ID')) . sprintf('<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>', esc_url(get_author_posts_url(get_the_author_meta('ID'))), esc_attr(sprintf(esc_html__('View all posts by %s', 'sport-ak'), get_the_author())), get_the_author()) . '</span>';
            break;
        case 'post_like':
        ...
        default:
            if (isset($options['meta_fields']) && in_array($name, array_filter($options['meta_fields']))) {
                ...
                if (in_array($slug, $taxonomies)) {
                    $term_list = get_the_term_list(0, $slug, '', esc_html__(', ', 'sport-ak'), '');
                    if ($term_list) {
...
function azexo_embed_oembed_html($html, $url, $attr, $post_ID) {
    if (preg_match('/src="([^"]*)"/', $html, $matches)) {
        ...
        $html = preg_replace('/src="[^"]*"/', 'src="' . htmlentities(esc_url(azexo_unparse_url($oembed_src))) . '"', $html);
    }
