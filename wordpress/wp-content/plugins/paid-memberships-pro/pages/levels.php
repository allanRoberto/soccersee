                ...
                <div id="layers-widget-pricing-table-4-1" class="pricing-plan pricing-plan-horizontal">
                    ...
                    <footer class="pricing-plan-footer column-flush span-3">
                    	<?php if(empty($current_user->membership_level->ID)) { ?>
								<a class="button btn-regular btn-regular pricing-button" href="<?php echo pmpro_url("checkout", "?level=" . $level->id, "https")?>"><?php _e('Select', 'pmpro');?></a>
							...
							<?php } elseif($current_level) { ?>      
								...
								<?php
									...
									if( pmpro_isLevelExpiringSoon( $current_user->membership_level) && $current_user->membership_level->allow_signups ) {
										?>
											<a class="button btn-regular btn-regular pricing-button" href="<?php echo pmpro_url("checkout", "?level=" . $level->id, "https")?>"><?php _e('Renew', 'pmpro');?></a>
										<?php
									} else {
										?>
