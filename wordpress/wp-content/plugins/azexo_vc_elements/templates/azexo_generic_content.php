...
if (!empty($sr)) {
    wp_enqueue_script('azvc-scrollReveal');
    $sr = 'data-sr="' . $sr . '"';
}
...
if (!function_exists('azexo_generic_content_image')) {
    ...
    function azexo_generic_content_image($image, $img_size, $media_link_click, $popups, $media_link) {
        ...
        if (!empty($image_url)) {
            ...
            $media_link = array_filter(vc_build_link($media_link));
            if ($img_size == 'full') {
...
if (!empty($title)) {
    ...
    $title_link = array_filter(vc_build_link($title_link));
...
if (!empty($content) && ($content != "<br />\n")) {
    ...
    print wpb_js_remove_wpautop($content, true);
    print "</div>\n";
}

if (!empty($footer)) {
    print '<div class="entry-footer">';
    $footer_link = array_filter(vc_build_link($footer_link));
